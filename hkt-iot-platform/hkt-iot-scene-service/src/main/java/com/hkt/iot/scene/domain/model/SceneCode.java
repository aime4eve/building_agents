...
public class SceneCode {
    ...
    private SceneCode(String value) {
        ...
        if (!CODE_PATTERN.matcher(trimmed).matches()) {
            throw new IllegalArgumentException("场景编码只能包含字母、数字、下划线和连字符");
        }
        if (trimmed.length() > 50) {
            throw new IllegalArgumentException("场景编码不能超过50个字符");
        }
    ...
    public static SceneCode of(String value) {
        return new SceneCode(value);
    }
