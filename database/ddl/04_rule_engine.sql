...
-- ============================================================================
-- 5. 规则动作执行日志表 (rule_action_execution_log)
-- 说明: 规则动作执行详细日志表
-- ============================================================================
...
2. 分表策略：
   - rule表：一般数据量不大，不需分表
   - rule_execution_log表：数据量大，按时间分区
     * 按月分区：PARTITION BY RANGE (TO_DAYS(triggered_at))
     * 保留近期数据（如3个月），历史数据归档
   - rule_action_execution_log表：同执行日志表，按时间分区

3. 索引优化：
   - rule表：建立复合索引 (tenant_id, rule_status, is_enabled)
   - rule_condition表：按rule_id建立索引
   - rule_action表：按rule_id建立索引
   - rule_execution_log表：建立时间范围查询索引 (triggered_at)

4. 数据归档策略：
   - 执行日志保留90天，超过90天的数据归档到历史表或对象存储
   - 已归档的执行日志可通过ES检索
   - 重要规则执行记录可长期存储
...
/*
          ...
          ▼               ▼               ▼
  ┌──────────────┐ ┌─────────────┐ ┌─────────────────┐
  │rule_condition│ │ rule_action │ │  rule_variable  │
  │──────────────│ │─────────────│ │─────────────────│
  │ id (PK)      │ │ id (PK)     │ │ id (PK)         │
  │ rule_id (FK) │ │ rule_id(FK) │ │ rule_id (FK)    │
  │ condition_   │ │ action_     │ │ variable_code   │
  │   type       │ │   type      │ │ (UK)            │
  │ device_id    │ │ target_     │ └─────────────────┘
  │              │ │   device_id │
  └──────────────┘ └─────────────┘
                          ...
                          ▼
              ┌───────────────────────────────┐
              │   rule_action_execution_log    │
              │───────────────────────────────│
              │ id (PK)                       │
              │ rule_execution_id (FK)        │
              │ rule_action_id (FK)           │
              │ action_execution_id (UK)      │
              └───────────────────────────────┘
                          ▲
                          │ rule_execution_id (FK)
                          │
