...
2. 分表策略：设备表按设备数量分表
   - 单表超过500万条记录时考虑分表
   - 分表键：device_id 或 tenant_id + device_id
   - 分表算法：hash(device_id) % 表数量

3. 时序数据策略：
   - device_telemetry_snapshot 只存储最新快照
   - 历史时序数据存储在 InfluxDB/TDengine
   - 按时间分区：day/month
   - 保留策略：7天/30天/90天/1年

4. 索引优化：
   - 所有表添加 tenant_id 索引（租户数据隔离）
   - 设备表按 device_sn, device_status, online_status 建立组合索引
   - 快照表按 device_id 建唯一索引，保证每设备一条快照
*/
